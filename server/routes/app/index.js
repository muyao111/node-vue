module.exports = (app) => {
  ...
  const Ariticle = mongoose.model('Ariticle')
  ...
  router.get('/news/init', async (req, res) => {
    ...
    const newsTitles = [
      '貂蝉五五开黑节皮肤海报线稿方案票选结果公布',
      '元歌皮肤设计大赛首轮投票开启公告',
      '阿轲-暗夜猫娘皮肤优化海报线稿方案票选结果公布',
      '指尖乾坤的非遗传承人姚建萍|《营地·人物》',
      '花语花愿·赴花朝，蔡文姬花朝节带你畅游五大花海',
      '3月10日全服不停机更新公告',
      '3月10日英雄平衡调整丨亚瑟、张飞、艾琳增强',
      '3月4日体验服停机更新公告',
      '微信游戏视频灰度测试启动',
      '3月3日全服不停机更新公告',
      '貂蝉五五开黑节皮肤海报线稿方案票选结果公布',
      '元歌皮肤设计大赛首轮投票开启公告',
      '元歌皮肤设计大赛首轮投票开启公告',
      '夏洛特-永昼即将上线，拥有皮肤送专属语音',
      '阿轲-暗夜猫娘皮肤优化海报线稿方案票选结果公布',
      '貂蝉五五开黑节皮肤海报线稿方案票选活动开启',
      '今日预报丨南京Hero战长沙TES.A，苏州KSG对厦门VG',
      '今日预报丨天狼遇再作序，重庆狼队、广州TTG再战谁能取胜',
      '烟花盛典闪耀长沙，第五届王者荣耀全国大赛城市赛道开启',
      '今日预报丨为全新荣耀而战，南京Hero战成都AG谁能取胜',
      '今日预报丨上海EDGM迎虎而上，广州TTG能否胜武汉ES',
    ]
    ...
    await Ariticle.deleteMany({})
    await Ariticle.insertMany(newsList)
    res.send(newsList)
  })
  // 新闻列表接口
  ...
  // 文章详情接口
  router.get('/ariticles/:id', async (req, res) => {
    const data = await Ariticle.findById(req.params.id).lean()
    data.related = await Ariticle.find()
